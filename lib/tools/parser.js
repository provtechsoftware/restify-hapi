...
const parse = (query, model, options) => {
  let err = null;
  let valid = true;
  ...
  if (options.archive && options.archive.enabled === true) {
    requestQuery.query[options.archive.attribute] = true;
  } else if (model.schema.paths._archived) {
    requestQuery.query._archived = false;
  }
