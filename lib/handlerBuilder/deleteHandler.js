...
const exec = (Model, logger, request, reply, options) => {
  ...
  Model.find(query).exec((err, dbResources) => {
    if (err) {
      ...
      DependenciesHandler.handleDestroyDependencies(dbResources, options, (err) => {
        if (err) {
          return errorHandler(err);
        } else {
          if (options.archive && options.archive.enabled === true) {
            ...
            console.log('removing1', err);
            Model.update(query, fields).lean().exec((err) => {
              if (err) {
                return errorHandler(err);
              } else {
                return reply({}).redirect(options.prefix).code(200);
              }
            });
          } else {
            Model.remove(query).lean().exec((err) => {
              console.log('removing2', err);
              if (err) {
                return errorHandler(err);
              } else {
                return reply({}).redirect(options.prefix).code(200);
