...
const exec = (Model, logger, request, reply, options) => {
  const result = Parser.parse(request.query, Model);
  ...
  const execQuery = (next) => {
    if (options.populate !== false) {
      Model.find(params.query, params.project, dbOptions)
        .sort(params.sort).populate(dispatchPopulates(options.populate))
        .lean().exec((err, dbResources) =>
          responseHandler(err, dbResources, next));
    } else {
      Model.find(params.query, params.project, dbOptions)
        .sort(params.sort)
        .lean().exec((err, dbResources) => responseHandler(err, dbResources, next));
    }
  };
