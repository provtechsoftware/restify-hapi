...
const exec = (Model, logger, request, reply, options) => {
  ...
  if (options.populate !== false) {
    Model.findById(request.params.id).populate(dispatchPopulates(options.populate)).select(params.project).lean().exec(responseHandler);
  } else {
    Model.findById(request.params.id).select(params.project).lean().exec(responseHandler);
  }
